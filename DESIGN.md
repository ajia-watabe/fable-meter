...
- ドロップダウン(日本語 or 英語。§5.1。ラベルは全角幅を2桁として計算し、等幅 `font=Menlo size=12` で桁を揃える):
  ...
  予測 Fable      リセット時点 ~34%     ← 履歴が足りなければ両方まとめて「予測: データ収集中(あと約2時間)」
  予測 週間       リセット時点 ~27%
  ---
  取得: 21:05:12 (3分前)
...
#### 0. 対象の指標(`PROJECTION_METRICS`)

パイプライン(窓の切り出し・成分A・成分B・外挿)は**指標名を引数に取る純関数**で、
`fable` と `seven_day`(= 週間(全モデル))の 2 つに同じものを適用する。
`history.jsonl` には最初から両方の % が入っているので、**スキーマは変えない**。
指標ごとに違うのは 2 点だけ:

- **値の列**: 傾きも活動カーブも % 低下によるリセット検出も、その指標の値だけを見る。
- **`resets_at`**: 残り時間はその指標自身の `resets_at`(state.json の
  `data.fable.resets_at` / `data.seven_day.resets_at`)で測る。
...
下記の「最近接の分に丸める」`reset_key` では**同じキーになる**(14:59:59+30s も
15:00:00+30s も 15:00 の分に落ちる)。したがって履歴に `seven_day_resets_at` を
足す必要はなく、既存の 1 列で両方を判別できる。

#### 1. 窓の切り出し(共通)

`history.jsonl` を読み、**現在の窓の点だけ**を取る。新しい順に遡り、`fable_resets_at` が
現在の `resets_at` と違う点、または(時系列で見て)**その指標の** % が下がる直前の点で打ち切る。
...
#### 5.5 行の組み立て(`projection_rows`)

指標ごとに 1 行、3 枠の行と同じ `pad_label`(16 セル)でラベルを揃えて出す。
桁が揃うように**予測行も等幅**(`font=Menlo size=12`)にする。色もアクションも付けない。
...
- **両方とも収集中**なら 6 の行を **1 行だけ**出す(サンプルの時刻が同じなので
  カウントダウンも同じ。同じ文言を 2 行並べても情報が増えない)。
- 片方だけ外挿できるとき(履歴の `seven_day` が欠測している等)は、
  出せる方は値の行、出せない方はラベル付きの `予測 週間       データ収集中(...)` を出す。
- その指標の `resets_at` が不明/過去なら**その行だけ**落とす。両方落ちれば行は 0 本。
- **通知(§4.2)は Fable のみ**。週間は表示だけで、通知の対象を増やさない。
