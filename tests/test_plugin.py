...
class RenderTest(unittest.TestCase):
    def test_dropdown_contains_rows_and_actions(self):
        ...
        self.assertIn("Refresh now | bash=/p/python3 param1=/r/fetch.py "
    ...
    def test_error_row_rendered_in_red(self):
        ...
        self.assertIn("Error: token_expired", out)
        self.assertIn(plugin.COLOR_ERROR, out)

    def test_missing_state_renders_error_row(self):
        out = plugin.render(None, NOW)
        self.assertIn("Error: state.json missing or unreadable", out)


class FormatTest(unittest.TestCase):
    def test_fmt_duration(self):
        self.assertEqual(plugin.fmt_duration(0), "0m")
        self.assertEqual(plugin.fmt_duration(90), "1m")
        self.assertEqual(plugin.fmt_duration(3600 * 3 + 60 * 21), "3h 21m")
        self.assertEqual(plugin.fmt_duration(86400 * 5 + 3600 * 2), "5d 2h")
